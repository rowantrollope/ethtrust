...
<template>
    <div class="flex items-center space-x-5 ">
        <img alt="cert" width="120" src="../assets/money.png">
        <h3 class="flex-1 text-5xl font-black leading-6 text-gray-900">
            Create new trust
        </h3>
    </div>
    ...
    <ol class="border border-gray-300 mt-10 rounded-md divide-y divide-gray-300 md:flex md:divide-y-0">
      <li v-for="(step, stepIdx) in steps" :key="step.name" class="relative md:flex-1 md:flex">
        <a v-if="step.status === 'complete'" :href="step.href" class="group flex items-center w-full">
          <span class="px-6 py-4 flex items-center text font-medium">
            ...
            <span class="ml-4 text font-medium text-gray-900">{{ step.name }}</span>
          </span>
        </a>
        <a v-else-if="step.status === 'current'" :href="step.href" class="px-6 py-4 flex items-center text font-medium" aria-current="step">
          <span class="flex-shrink-0 w-10 h-10 flex items-center justify-center border-2 border-indigo-600 rounded-full">
            <span class="text-indigo-600">{{ step.id }}</span>
          </span>
          <span class="ml-4 text font-medium text-indigo-600">{{ step.name }}</span>
        </a>
        <a v-else :href="step.href" class="group flex items-center">
          <span class="px-6 py-4 flex items-center text font-medium">
            ...
            <span class="ml-4 text font-medium text-gray-500 group-hover:text-gray-900">{{ step.name }}</span>
          </span>
        </a>
  ...
  <div class="bg-white border-2 shadow overflow-hidden sm:rounded-lg mt-10 ">
    <div class="px-4 py-5 sm:px-6 flex items-center space-x-5 ">
      <h3 class="text-2xl font-medium leading-6 text-gray-900">
        Step 2: Enter Trust Details <br/> <span class="mt-1 font-base max-w-2xl text-sm text-gray-600">
        Enter the name of the trust, the beneficiary account, maturity date and etc
      </span>
      </h3>
    ...
    <div class="border-t border-gray-200 px-4 py-5">
      <dl class="sm:divide-y sm:divide-gray-200">
        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          <dt class="text font-medium text-gray-600">
            Trust name
          </dt>
        ...
        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          ...
          <input type="number" 
            ...
            <div class="border-t pt-10 flex space-x-5">
                <Button class="flex-1 btn text-xl btn-primary-outline">Previous</Button>
                <Button class="flex-1 btn text-xl btn-success">Next > </Button>
            </div>
    </div>
  </div>
...
const steps = [
  { id: '01', name: 'Create Beneficiary Account', href: '#', status: 'complete' },
  { id: '02', name: 'Trust Details', href: '#', status: 'current' },
  { id: '03', name: 'Adding Funding', href: '#', status: 'upcoming' },
]
...
<style scoped>
.input-field {
    @apply md:flex-1  -mt-2 -ml-3 block border-0 w-full min-w-0 focus:ring-indigo-500 ;
}
</style>
